    ...
    while (1)
    {
        ...
        if (FD_ISSET(pSelf->m_vdrv_cmdpipe[0], &rfds))
        {
            ...
            if (cmd == OMX_VdrvCommand_PrepareVdecLib) //now omx_vdec's m_state = OMX_StateLoaded, OMX_StateLoaded->OMX_StateIdle
            {
                ...
                if (pSelf->m_streamInfo.nWidth > 1920 && pSelf->m_streamInfo.nHeight > 1088)
                {
        ...
        //*Buffer processing
        // Only happens when the component is in executing state.
        if ((pSelf->m_state == OMX_StateExecuting || pSelf->m_state == OMX_StatePause) && (pSelf->m_decoder != NULL) && (nVdrvResolutionChangeFlag == 0))
        {
#if (OPEN_STATISTICS)
