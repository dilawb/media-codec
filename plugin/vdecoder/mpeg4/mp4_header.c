...
{
	int32_t i;
	int32_t flag = (mp4_state->userdata_codec_version == 500) && (mp4_state->userdata_build_number >= 370) && (mp4_state->userdata_build_number <= 413);
...
*/
	...
	{
		mp4_state->hdr.resync_length = (mp4_state->hdr.prediction_type == MP4_I_VOP) ? 17 : (16 + mp4_state->hdr.fcode_for);
	}
...
int16_t mp4_getpackethdr(MP4_STREAM *_ld, MP4_STATE *_mp4_state)
{
	...
	if (mp4_state->hdr.shape != BINARY_SHAPE_ONLY)
	{
		...
		if (mp4_state->hdr.prediction_type != MP4_B_VOP)
		{
			...
			mp4_state->hdr.use_intra_dc_vlc = mp4_get_use_intra_dc_vlc(mp4_state->hdr.quantizer,
